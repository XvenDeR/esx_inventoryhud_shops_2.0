            ...
            for i = 1, #list, 1 do
				if list[i].name == item then
					...
					if xPlayer.getMoney() >= totalPrice then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = '¡Te falta dinero!' })
					end
				end
            end
        else
            TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = '¡Ya no puedes cargar mas cosas!' })
        end
	end
				...
				if list[i].name == item then
					...
					if not targetWeapon then
						...
						if xPlayer.getMoney() >= totalPrice then
							...
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Compraste una '..list[i].label })
						else
							TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = '¡Te falta dinero!' })
						end
					else
						TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = '¡Ya tienes esta arma!' })
					end
				end
            end
        else
		...
		for i = 1, #list, 1 do
			if list[i].name == item then
				...
				if targetWeapon then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = '¡No posees un arma para este tipo de municion!' })
				end
            end
        end
    end
...
AddEventHandler('suku:buyLicense', function ()
	...
	if xPlayer.get('money') >= Config.LicensePrice then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = '¡Has aprobado para obtener una licencia de armas!' })
		TriggerEvent('esx_license:addLicense', _source, 'weapon', function ()
			GetLicenses(_source)
		end)
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = '¡No tienes el dinero!' })
	end
end)